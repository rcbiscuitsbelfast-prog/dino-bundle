...
@onready var unlimited_lives_toggle: CheckButton = $VBoxContainer/UnlimitedLivesContainer/UnlimitedLivesToggle
@onready var back_button: Button = $VBoxContainer/BackButton

func _ready() -> void:
    # Ensure SettingsManager is loaded
    if not SettingsManager:
        push_error("SettingsManager not loaded!")
        return
...
func load_current_settings() -> void:
    ...
    unlimited_lives_toggle.button_pressed = SettingsManager.get_unlimited_lives()
...
func connect_signals() -> void:
    ...
    if unlimited_lives_toggle:
        unlimited_lives_toggle.toggled.connect(_on_unlimited_lives_toggled)
    if back_button:
        back_button.pressed.connect(_on_back_pressed)
    
    # Connect to SettingsManager signals for real-time updates
    if SettingsManager and SettingsManager.has_signal("volume_changed"):
...
func _on_unlimited_lives_toggled(enabled: bool) -> void:
    SettingsManager.set_unlimited_lives(enabled)
    if enabled:
        unlimited_lives_toggle.text = "On"
    else:
        unlimited_lives_toggle.text = "Off"
